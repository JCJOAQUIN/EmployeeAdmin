        ...
        @endif
            ...
            <div class="mt-4 flex justify-center space-x-8">
                <a href="{{route('schedules.create')}}">
                    <button class="w-32 p-2 bg-primary hover:bg-primarySoft rounded-full text-lightSoft hover:text-light font-semibold" type="button">Create</button>
                </a>
                <a href="{{route('schedules.assign')}}">
                    <button class="w-32 p-2 bg-third hover:bg-secondarySoft rounded-full text-lightSoft hover:text-light font-semibold" type="button">Assign</button>
                </a>
            </div>
            <div class="text-2xl font-semibold pb-1 pl-2 mt-8 border-b-4 border-primary text-teal-600">
                @if (isset($requests))
                    <i class="fa-regular fa-calendar-check"></i> Assign schedules edition
                @else
                    <i class="fa-regular fa-calendar-check"></i> Assign schedules
                @endif
            </div>
            ...
            <div class="grid md:grid-cols-3 sm:grid-cols-2 grid-cols-1 bg-light bg-opacity-60 p-6 sm:p-8 rounded-md gap-x-8">
                ...
                @php
                    $options = collect();
                @endphp
			    @component('components.inputs.select', ["label" => "Schedule:", "options" => $options, "attributeSelect" => "name=\"schedule\" multiple=\"multiple\" id=\"schedule\""]) @endcomponent
			    @component('components.inputs.inputCombo', ["classCombo" => "col-span-1", "label" => "Start time:",  "attributeInput" => "disabled name=\"startTime\" type=\"time\" placeholder=\"Start time\" id=\"startTime\""]) @endcomponent
			    @component('components.inputs.inputCombo', ["classCombo" => "col-span-1", "label" => "End time:",  "attributeInput" => "disabled name=\"endTime\" type=\"time\" placeholder=\"End time\" id=\"endTime\""]) @endcomponent
            </div>
