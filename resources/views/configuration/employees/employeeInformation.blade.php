...
@section('content')
    <div class="mx-4">
        ...
        <div class="text-2xl font-semibold pb-1 pl-2 mt-8 border-b-4 border-primary text-teal-600">
            <i class="fa-solid fa-address-card"></i> Employee Information
        </div>
    </div>
    <div class="flex justify-center lg:text-base md:text-sm sm:text-xs text-sm mt-12 mx-4">
        <div class="lg:w-4/5 w-full border-thirdSoft rounded-md p-4 shadow-md">
            <div class="grid md:grid-cols-6 grid-cols-1 border-b-2 pb-2 md:gap-0 gap-2">
                ...
                <div class="col-span-4 grid place-content-center">@component('components.labels.label', ["label" => isset($requests->user) ? $requests->user->fullName() : ""]) @endcomponent</div>
                <div class="col-span-1 grid place-content-center">
            ...
            <div class="grid sm:grid-cols-2 grid-cols-1 py-4 md:px-0 gap-2">
                <div class="col-span-1 grid md:grid-cols-2 grid-cols-2">
                    @component('components.labels.label', ["classLabel" => "text-third font-bold", "label" => "Clabe:"]) @endcomponent
                    @component('components.labels.label', ["label" => $requests->clabe]) @endcomponent
                </div>
                <div class="col-span-1 grid md:grid-cols-2 grid-cols-2">
                    @component('components.labels.label', ["classLabel" => "text-third font-bold", "label" => "Alias:"]) @endcomponent
                    @component('components.labels.label', ["label" => $requests->alias]) @endcomponent
                </div>
                <div class="col-span-1 grid md:grid-cols-2 grid-cols-2">
                    @component('components.labels.label', ["classLabel" => "text-third font-bold", "label" => "User:"]) @endcomponent
                    @component('components.labels.label', ["label" => isset($requests->user->name) ? $requests->user->name : ""]) @endcomponent
                </div>
                <div class="col-span-1 grid md:grid-cols-2 grid-cols-2">
                    @component('components.labels.label', ["classLabel" => "text-third font-bold", "label" => "Area:"]) @endcomponent
                    @component('components.labels.label', ["label" => $requests->area]) @endcomponent
                </div>
                <div class="col-span-1 grid md:grid-cols-2 grid-cols-2">
                    @component('components.labels.label', ["classLabel" => "text-third font-bold", "label" => "Department:"]) @endcomponent
                    @component('components.labels.label', ["label" => $requests->department]) @endcomponent
                </div>
            </div>
        </div>
    </div>
